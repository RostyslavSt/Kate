...
/****************************************************************************************
* Распечатать дерево при помощи картинок. 
*/
Tree.prototype.getGIF = function (name)
{
   return Tree.getGIF(this, name);
}
/****************************************************************************************
...
* Получить массив узлов дерева tr, имеющих свойство prop в значении val
*/
Tree.arrProp = function(tr, prop, val)
{
   let ar = [];
   if(tr[prop]===val)                             // есть такое свойство
      ar.push(tr);
   if(tr.ar)
   for(let i=tr.ar.length; i-- ; )
      ar = ar.concat(this.arrProp(tr.ar[i], prop, val));
   return ar;
}
...
/****************************************************************************************
* Распечатать дерево при помощи картинок. При стартовом запуске calc отсутсвует
*/
Tree.getGIF = function (tr, name, calc)
{
   if(!name)
      name="";
   if(!calc){                                     // это первый запуск (не рекурсия)    
      let st  = '<div class="tree">\n'; 
      let lst = this.getGIF(tr, name, true);      // получаем массив строк, котрые
      for(let i=0; i < lst.length; i++)           // сворачиваем в одну стрку
         st += '<div class="blk">'+lst[i]+"</div>";                                
      return st+'\n</div>';
   }
   
   if(!tr.ar || tr.ar.length===0)                 // это лист, возвращаем только имя
      return [this.href(name,tr.id,1) + (tr.chk?Tree.img(13):Tree.img(10)) + '</a>'
             +this.href(name,tr.id,2) + tr.nm + '</a>'];
   
   let lst = [];                                  // список строк, который вернёт узел
   lst.push(
      this.href(name,tr.id,1) 
   + (tr.opn?(tr.chk===0?Tree.img(8):(tr.chk===1?Tree.img(11):Tree.img(14)))
            :(tr.chk===0?Tree.img(9):(tr.chk===1?Tree.img(12):Tree.img(15)))
      ) + '</a>' +this.href(name,tr.id,2) + tr.nm + '</a>'
   );      
   for(let i=0; i < tr.ar.length; i++){           // по всем потомкам
      let n = tr.ar[i];
      if(n.vis===false)
         continue;
      let nxt = Tree.getGIF(n, name, true);       // получаем массив строк потомка
      for(let j=0; j < nxt.length; j++){          // добавляем его в массив этого узла
         let ch = Tree.img(0);                    // символ линий, соединяющих узлы         
         if(j===0){                               // первая строка (с именем потомка)
            if(i+1 !== tr.ar.length)              // не последний потомок
               ch = tr.ar[i].ar? this.href(name,n.id,0)+(n.opn?Tree.img(7):Tree.img(5))+'</a>'
                  : Tree.img(3);  // " + " для папки и " |-"
            else                                  // последний потомок
               ch = tr.ar[i].ar? this.href(name,n.id,0)+(n.opn?Tree.img(6):Tree.img(4))+'</a>'
                  : Tree.img(2);  // тоже, но нет линии вниз
         }
         else if(i+1 !== tr.ar.length)            // соединительная линия вниз
            ch = Tree.img(1);            
...
{
   dir: "../../_im/",
   ar:  [ "_empt.gif", "_l101.gif", "_l110.gif", "_l111.gif", // 0,1,2,3 линии
          "_p110.gif", "_p111.gif", "_m110.gif", "_m111.gif", // 4,5,6,7 плюсы,минусы
          "_flop.gif", "_flcl.gif", "_page.gif",              // 8,9,10  папка,страница
          "_ckop.gif", "_ckcl.gif", "_pgck.gif",              // 11,12,13 помеченные 
          "_isop.gif", "_iscl.gif",                           // 14,15  содержат помеч.
        ],
};
Tree.img = function(id){
   return '<img src="'+this.gifs.dir+this.gifs.ar[id]+'"/>';
}
Tree.href = function(name, id, knd){  
   return '<a href="javascript:'+name+'.clk('+id+','+knd+');">';
}
/****************************************************************************************
* Получить графическое представление дерева в svg-формате.
