...
/****************************************************************************************
* Провести одну итерацию поиска направленного пути; 
* возвращает -1, если пути нет, 1 - если найден, иначе (продолжает поиск) - 0
*/
Graph.prototype.searchPathDirRun = function()
{
   if(!this.lstNxt.length)
      return -1;                                  // поиск закончился неудачей
   
   let k = this.lstNxt.shift().n;                 // берём узел из начала списка
   let n = this.nodes[k];
   if( k!== this.nodeSrc) n.chk = 3;              // красим его (для визуализации)
   ...
   for(let i=n.go.length; i--; ){                 // для каждого соседа nxt
      ...
      if(this.lstWas[nxt] !== undefined )                 
         continue;                                // этот узел уже был
                                                  // в начало сортированного списка:
      this.lstNxt.add({n:nxt, d:this.searchDist(nxt,this.nodeDes)}); 
      this.lstWas[nxt] = k;                       // помним, что в n.go[i] попали из узла k
...
/****************************************************************************************
* Закончить поиск направленного пути и вернуть
*  массив  последовательности узлов от финального к начальному (путь).
*/
Graph.prototype.searchPathDirEnd = function()
{
   return this.searchPathEnd();
}
/****************************************************************************************
* Подготовиться к двухстороннему поиску пути от узла i к узлу j
*/
Graph.prototype.search2PathBeg = function(i, j)
{
   if(i===j)
      return  1;
   if(!this.nodes[i].go || this.nodes[i].go.length===0)
   ...
   this.lstWas1 = {};         // "дерево бинарного поиска" просмотренных узлов от i 
   this.lstWas2 = {};         // "дерево бинарного поиска" просмотренных узлов от j    
   this.lstNxt1 = new List(); // список узлов границы поиска от i
   this.lstNxt2 = new List(); // список узлов границы поиска от j   
   this.nodeSrc = i;          // начальный узел
   this.nodeDes = j;          // целевой узел
   this.nodeCnt = -1;         // граничный узел
   
   this.set("chk", 0);        // для визуализации всем узлам пометку chk:0
   this.nodes[i].chk = 1;
   this.nodes[j].chk = 1;
   
   this.lstNxt1.push(i);      // помещаем стартовый узел в список
   this.lstWas1[i] = i;
   
   this.lstNxt2.push(j);      // помещаем финальный узел в список
   this.lstWas2[j] = j;   
   
   return 0;
}
/****************************************************************************************
* Провести одну итерацию двухстороннего поиска пути; 
* возвращает -1, если пути нет, 1 - если найден, иначе (продолжает поиск) - 0
*/
Graph.prototype.search2PathRun = function()
{
   if(!this.lstNxt1.length)
      return -1;                                  // поиск закончился неудачей
   
   let k1 = this.lstNxt1.shift();
   let n1 = this.nodes[k1];
   if( k1 !== this.nodeSrc ) n1.chk = 3;
                       
   let k2 = this.lstNxt2.shift();
   let n2 = this.nodes[k2];
   if( n2 && k2 !== this.nodeDes ) n2.chk = 3;   
   
   if(n1.go)   
   for(let i=n1.go.length; i--; ){
      let nxt = n1.go[i];
     
      if(this.lstWas1[nxt] !== undefined )                 
         continue;                                // этот узел уже был
      
      this.lstNxt1.push(nxt);
      this.lstWas1[nxt] = k1;                     // в n.go[i] попали из узла k
      if(nxt === this.nodeDes || this.lstWas2[nxt] !== undefined){           
         this.nodeCnt = nxt;                      // узел связи
         return 1;                                // нашли!!!
      }
   ...
   if(n2 && n2.on)    
   for(let i=n2.on.length; i--; ){
      let nxt = n2.on[i];
     
      if(this.lstWas2[nxt] !== undefined)                 
         continue;                                // этот узел уже был
      
      this.lstNxt2.push(nxt);
      this.lstWas2[nxt] = k2;                     // в n.on[i] попали из узла k2
      if(nxt === this.nodeDes || this.lstWas1[nxt] !== undefined){
         this.nodeCnt = nxt;                      // узел связи         
         return 1;                                // нашли!!!
      }
      this.nodes[nxt].chk = 2;                    // помечаем узел для визуализации
   ...
   return 0;                                      // продалжаем поиск
}
/****************************************************************************************
* Закончить поиск пути, вернуть
* массив  последовательности узлов от начального к финальному.
*/
Graph.prototype.search2PathEnd = function()
{
   let path = [];                                 // обнуляем массив
   if(this.nodeCnt < 0)   
      return path;                                     // путь не был найден
   
   let k = this.nodeCnt;                          
   path.push(k);                                  // начиная от целевого узла,
   while( (k = this.lstWas1[k]) !== this.nodeSrc )// составляем путь
      path.push(k);
   path.push(k);   
   path.reverse();
   k = this.nodeCnt;
   while( (k = this.lstWas2[k]) !== this.nodeDes )// составляем путь
      path.push(k);   
   path.push(k);   
      
   this.lstWas1 = {};   this.lstNxt1 = null;      // очищаем списки   
   this.lstWas2 = {};   this.lstNxt2 = null;      // очищаем списки      
